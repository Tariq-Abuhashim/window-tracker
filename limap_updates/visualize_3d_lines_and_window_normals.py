...
def parse_args():
    ...
    arg_parser.add_argument('--normals_file', type=str, default=None, help="input normals file. Format supported now: .json")
...
def vis_reconstruction(linetracks, imagecols, normals, centers, mode="open3d", n_visible_views=4, ranges=None, scale=1.0, cam_scale=1.0):
    if mode == "open3d":
    ...
    VisTrack.report()
    VisTrack.vis_reconstruction(imagecols, normals=normals, centers=centers, n_visible_views=n_visible_views, ranges=ranges, scale=scale, cam_scale=cam_scale)

def main(args):
    lines, linetracks = limapio.read_lines_from_input(args.input_dir)
    normals, centers = limapio.read_normals_from_input(args.normals_file)
    ranges = None
    ...
    if args.imagecols is None:
        ...
        vis_reconstruction(linetracks, imagecols, normals=normals, centers=centers, mode=args.mode, n_visible_views=args.n_visible_views, ranges=ranges, scale=args.scale, cam_scale=args.cam_scale)
