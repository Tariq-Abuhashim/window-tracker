...
void LoopClosing::CorrectLoopWithObjects() {
	...
	cout << "7" << endl;
    // Optimize graph
    Optimizer::OptimizeEssentialGraph(pMap, mpLoopMatchedKF, mpCurrentKF, NonCorrectedSim3, CorrectedSim3,
...
bool LoopClosing::ComputeSim3()
{
    ...
    vector<KeyFrame*> vpLoopConnectedKFs = mpLoopMatchedKF->GetVectorCovisibleKeyFrames();
    vpLoopConnectedKFs.push_back(mpLoopMatchedKF);
    ...
    {
        for(int i=0; i<nInitialCandidates; i++)
            if(mvpEnoughConsistentCandidates[i]!=mpLoopMatchedKF) // was mpMatchedKF in ORB_SLAM2
                mvpEnoughConsistentCandidates[i]->SetErase();
        return true;
    }
    else
...
void LoopClosing::RunGlobalJointBundleAdjustment(unsigned long nLoopKF) {
    ...
    Optimizer::GlobalJointBundleAdjustemnt(pMap, 10, &mbStopGBA, nLoopKF, false);
