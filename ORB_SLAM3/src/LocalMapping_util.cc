...
//void LocalMapping::SetObjectMapper(ObjectMapping *pObjectMapper)
//{
//    mpObjectMapper=pObjectMapper;
//}
...
void LocalMapping::CreateNewMapObjects()
{
	...
	for (size_t i = 0; i < mvpObjectDetections.size(); i++)
	{
		...
		if (det->nRays == 0)
		{
			cout << "    Skipping detection with no rays." << endl;
		        continue;
		}
		if (!det->isNew)
		{
		...
		{
			cout << "    Skipping detection: reconstructed object is not good." << endl;
			continue;
		}
...
void LocalMapping::CreateNewObjectsFromDetections()
{
    ...
    for (int det_i = 0; det_i < mvpObjectDetections.size(); det_i++)
    {
        ...
        auto pNewObj = new MapObject(mpCurrentKeyFrame, pCurrentMap);
        mpCurrentKeyFrame->AddMapObject(pNewObj, det_i);
        mpAtlas->AddMapObject(pNewObj);
...
//            continue;
        ...
        if (n_valid_points >= 50 && n_rays > 20)
        {
            ...
            Eigen::VectorXf code = Eigen::VectorXf::Zero(64);   // was Eigen::Vector<float, 64>
            if (code_len == 32)
            {
                auto code_32 = pyMapObject.attr("code").cast<Eigen::VectorXf>();  // was Eigen::Vector<float, 32>
                code.head(32) = code_32;
            }
            else
            {
                code = pyMapObject.attr("code").cast<Eigen::VectorXf>(); // was Eigen::Vector<float, 64>
            }
