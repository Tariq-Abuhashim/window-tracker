...
class Optimizer(object):
    ...
    def reconstruct_object(self, t_cam_obj, pts, rays, depth, code=None):
        ...
        """
        print("reconstruct_object ...")
        try:
            # Validate inputs
            if t_cam_obj is None or pts is None or rays is None or depth is None:
                raise ValueError("Invalid input to reconstruct_object: one or more inputs are None.")
    
            # Initialize variables - Always start from zero code
            if code is None:
            ...
            for e in range(self.num_iterations_joint_optim):
                ...
                if sdf_rst is None:
                    print("SDF computation returned None.")
                    return ForceKeyErrorDict(t_cam_obj=None, code=None, is_good=False, loss=loss, error="SDF computation failed")
                de_dsim3_sdf, de_dc_sdf, res_sdf = sdf_rst
                
                robust_res_sdf, sdf_loss, _ = get_robust_res(res_sdf, self.b2)
                if math.isnan(sdf_loss):
                    return ForceKeyErrorDict(t_cam_obj=None, code=None, is_good=False, loss=loss, error="Render computation failed")
                ...
                if render_rst is None:
                    print("Render computation returned None.")
                ...
                if math.isnan(render_loss):
                    return ForceKeyErrorDict(t_cam_obj=None, code=None, is_good=False, loss=loss, error="NaN loss detected")
            ...
            t_cam_obj = torch.inverse(t_obj_cam)
            return ForceKeyErrorDict(t_cam_obj=t_cam_obj.numpy(), code=latent_vector.cpu().numpy(), is_good=True, loss=loss)
            
        except Exception as e:
            print(f"Error in reconstruct_object: {e}")
            return ForceKeyErrorDict(t_cam_obj=None, code=None, is_good=False, loss=0, error=str(e))
